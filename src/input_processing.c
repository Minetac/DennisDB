...
int compare_input_with_table(Table *table, char input_text[4096]) {
    //printf("Splitting Input Text..\n");
    char **items = input_text_split(input_text);
    //printf("Retrieving Column Types from Text..\n");
...
char **input_text_split(const char input_text[4096]) {
    ...
    int item_count = 0;
    if (input_text[0] != '(' || strlen(input_text) < 2) { printf("Wrong Input!\n"), exit(1); }
...
enum col_type *column_types_from_split(char **input_items) {
    ...
    if (types == NULL) { printf("Speicherzuweisung fehlgeschlagen!\n"); exit(1); }
    //printf("Inside column_types_from_split\n");
    for (int i = 0; i < MAX_COLS; i++) {
        if (input_items[i] == NULL) break;
        //printf("Aktuell bei I: %d, \n", i);
